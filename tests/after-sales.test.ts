...
import { parsePiecesCount, parseServicePlan, parseServiceType, validateServicePlanUpdate } from "@/lib/after-sales";
...
describe("after-sales plan validation", () => {
  it("accepts an optional dealer plan with maintenance, claim, and re-wrap limits", () => {
    ...
    expect(parseServicePlan(form)).toEqual({
      ...
      nextRecommendedDateOverride: null,
    });
  });
  ...
  it("preserves already-used dealer entitlements when a plan is edited", () => {
    const form = new FormData();
    form.set("maintenanceIncluded", "on");
    form.set("maintenanceIntervalMonths", "6");
    form.set("maintenanceVisitLimit", "4");
    form.set("claimIncluded", "on");
    form.set("claimPieceLimit", "5");
    form.set("nextRecommendedDateOverride", "2027-08-25");
    expect(() => validateServicePlanUpdate(parseServicePlan(form), { maintenanceUsed: 1, claimUsed: 2, rewrapUsed: 0 })).not.toThrow();

    form.delete("claimIncluded");
    form.delete("claimPieceLimit");
    expect(() => validateServicePlanUpdate(parseServicePlan(form), { maintenanceUsed: 1, claimUsed: 2, rewrapUsed: 0 })).toThrow("ปิดสิทธิ์เคลมไม่ได้");
  });
});
