...
export async function POST(request: Request) {
  ...
  try {
    ...
    const maintenanceDate = validIsoDate(requiredText(form, "maintenanceDate", "วันที่บริการ", 10, 10), "วันที่บริการ");
    ...
    const nextRaw = formText(form, "nextRecommendedDate");
    const nextRecommendedDate = nextRaw ? validIsoDate(nextRaw, "วันที่แนะนำครั้งถัดไป") : null;
    ...
    const warranty = await env.DB.prepare("SELECT id, dealer_id FROM warranties WHERE serial_code = ? LIMIT 1").bind(serialCode).first<WarrantyRow>();
    if (!warranty) return fail("ไม่พบบัตรรับประกัน", 404);
    ...
    for (const photo of photos) {
      ...
      await env.FILES.put(objectKey, photo.stream(), { httpMetadata: { contentType: photo.type }, customMetadata: { referenceCode, serialCode, dealerId: String(warranty.dealer_id) } });
      uploadedKeys.push(objectKey);
    }
    const results = await env.DB.batch([
      env.DB.prepare(`INSERT INTO maintenance_records (reference_code, warranty_id, dealer_id, maintenance_date, maintenance_type, performed_by, result_status, note, next_recommended_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .bind(referenceCode, warranty.id, warranty.dealer_id, maintenanceDate, maintenanceType, performedBy, resultStatus, note || null, nextRecommendedDate),
      ...photos.map((photo, index) => env.DB.prepare(`INSERT INTO media_assets (owner_type, owner_reference, object_key, original_name, content_type, size_bytes) VALUES ('maintenance', ?, ?, ?, ?, ?)`)
        .bind(referenceCode, uploadedKeys[index], photo.name.slice(0, 160), photo.type, photo.size)),
      env.DB.prepare(`INSERT INTO audit_logs (actor_email, actor_role, action, entity_type, entity_id, detail) VALUES (?, 'admin', 'maintenance.create', 'warranty', ?, ?)`)
        .bind(actor.email, serialCode, JSON.stringify({ referenceCode, maintenanceType, performedBy, resultStatus, dealerId: warranty.dealer_id, photoCount: photos.length })),
    ]);
    return Response.json({ ok: true, referenceCode, serialCode, recordId: Number(results[0]?.meta?.last_row_id ?? 0) }, { status: 201 });
  } catch (error) {
    ...
    return fail("ไม่สามารถบันทึก Maintenance ได้", 500);
  }
}
