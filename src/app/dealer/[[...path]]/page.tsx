...
export default async function DealerPage({ params, searchParams }: { params: Promise<{ path?: string[] }>; searchParams: Promise<DealerSearchParams> }) {
  ...
  const initialSerial = section === "register-warranty" || section === "maintenance" ? normalizePrefillSerial(query.serial) : "";
  ...
  if (section === "register-warranty") return <DashboardShell role="Dealer" title="เปิดใช้งานบัตรรับประกัน" active="register"><Panel title="สแกน QR แล้วเปิดใช้งาน" copy="ไม่ต้องรอสำนักงานใหญ่ ระบบอ่านรุ่นจาก QR สร้าง Serial และคำนวณวันหมดอายุจากวันที่ติดตั้งให้อัตโนมัติ"><WarrantyJourney current="dealer" /><DemoForm kind="dealer-register" initialSerial={initialSerial} /></Panel></DashboardShell>;
...
function WarrantyDetail({ detail }: { detail: { warranty: DealerWarrantyRecord; plan: DealerServicePlan | null; maintenance: DealerMaintenanceItem[]; media: DealerMediaItem[] } | null }) {
  ...
  return <div className="warranty-detail-stack">
    <section className="dashboard-panel warranty-detail-panel">
      ...
      <dl><div><dt>ลูกค้า</dt><dd>{warranty.customer_name || "รอลูกค้ากรอกข้อมูล"}</dd></div><div><dt>โทรศัพท์</dt><dd>{warranty.customer_phone || "-"}</dd></div><div><dt>อีเมล</dt><dd>{warranty.customer_email || "-"}</dd></div><div><dt>รถ</dt><dd>{[warranty.vehicle_make, warranty.vehicle_model].filter(Boolean).join(" ") || "รอลูกค้ากรอกข้อมูล"}</dd></div><div><dt>ทะเบียน</dt><dd>{warranty.vehicle_plate || "-"}</dd></div><div><dt>ติดตั้ง</dt><dd>{formatPortalDate(warranty.install_date)}</dd></div><div><dt>หมดอายุ</dt><dd>{warranty.expiry_date ? formatPortalDate(warranty.expiry_date) : "ตาม Policy"}</dd></div><div><dt>ดูแลครั้งถัดไป</dt><dd>{plan?.next_recommended_date ? formatPortalDate(plan.next_recommended_date) : "-"}</dd></div></dl>
      <div className="dealer-benefit-grid">
        <DealerBenefit label="Maintenance" included={Boolean(plan?.maintenance_included)} used={Number(plan?.maintenance_used ?? 0)} limit={plan?.maintenance_visit_limit ?? null} unit="ครั้ง" detail={plan?.maintenance_interval_months ? `ทุก ${plan.maintenance_interval_months} เดือน` : undefined} />
        <DealerBenefit label="เคลม" included={Boolean(plan?.claim_included)} used={Number(plan?.claim_used ?? 0)} limit={plan?.claim_piece_limit ?? null} unit="ชิ้น" />
        <DealerBenefit label="Re-wrap" included={Boolean(plan?.rewrap_included)} used={Number(plan?.rewrap_used ?? 0)} limit={plan?.rewrap_piece_limit ?? null} unit="ชิ้น" />
      </div>
      {plan?.plan_note && <p className="dealer-plan-note"><b>หมายเหตุแพ็กเกจ</b>{plan.plan_note}</p>}
      <div className="detail-actions"><Link className="button button-primary" href={`/r/${encodeURIComponent(warranty.serial_code)}`}>เปิดบัตรสาธารณะ <span>→</span></Link>{warranty.status === "pending_customer" && <Link className="button button-secondary" href={`/warranty/complete?serial=${encodeURIComponent(warranty.serial_code)}`}>หน้ากรอกข้อมูลลูกค้า <span>→</span></Link>}<Link className="button button-secondary" href={`/dealer/maintenance?serial=${encodeURIComponent(warranty.serial_code)}`}>เพิ่มบริการ / ใช้สิทธิ์ <span>→</span></Link></div>
...
function DealerBenefit({ label, included, used, limit, unit, detail }: { label: string; included: boolean; used: number; limit: number | null; unit: string; detail?: string }) {
  return <article className={included ? "" : "not-included"}><span>{label}</span><b>{included ? `${used}/${limit} ${unit}` : "ไม่รวม"}</b><small>{included ? `คงเหลือ ${Math.max(0, Number(limit) - used)} ${unit}${detail ? ` · ${detail}` : ""}` : "ร้านไม่ได้ให้สิทธิ์นี้"}</small></article>;
}
