...
import { DealerRequestForm, DemoForm, WarrantyQr } from "../../client-ui";
import { AccessDenied, requirePartnerAccess } from "../../partner-auth";
import { getDealerProfile, getDealerStats, getDealerTasks, getDealerWarranties, getDealerWarrantyDetail, type DealerWarrantyDetail as DealerWarrantyRecord, type DealerMaintenanceItem, type DealerMediaItem, type PortalRecord } from "../../../db/portal-data";
...
function WarrantyDetail({ detail }: { detail: { warranty: DealerWarrantyRecord; maintenance: DealerMaintenanceItem[]; media: DealerMediaItem[] } | null }) {
  ...
  const { warranty, maintenance, media } = detail;
  return <div className="warranty-detail-stack"><section className="dashboard-panel warranty-detail-panel"><header><div><p className="eyebrow">{warranty.product_model_code}</p><h2>{warranty.serial_code}</h2></div><StatusPill status={warranty.status.replaceAll("_", "-")} /></header><dl><div><dt>ลูกค้า</dt><dd>{warranty.customer_name}</dd></div><div><dt>โทรศัพท์</dt><dd>{warranty.customer_phone}</dd></div><div><dt>อีเมล</dt><dd>{warranty.customer_email || "-"}</dd></div><div><dt>รถ</dt><dd>{warranty.vehicle_make} {warranty.vehicle_model}</dd></div><div><dt>ทะเบียน</dt><dd>{warranty.vehicle_plate}</dd></div><div><dt>ติดตั้ง</dt><dd>{formatPortalDate(warranty.install_date)}</dd></div><div><dt>หมดอายุ</dt><dd>{warranty.expiry_date ? formatPortalDate(warranty.expiry_date) : "ตาม Policy"}</dd></div></dl><div className="detail-actions"><Link className="button button-primary" href={`/r/${encodeURIComponent(warranty.serial_code)}`}>เปิดบัตรสาธารณะ <span>→</span></Link><Link className="button button-secondary" href="/dealer/maintenance">เพิ่ม Maintenance <span>→</span></Link></div><WarrantyQr cardPath={`/r/${encodeURIComponent(warranty.serial_code)}`} serial={warranty.serial_code} /></section><section className="detail-subgrid"><div className="dashboard-panel"><header><h2>Maintenance history</h2></header>{maintenance.length ? maintenance.map((item) => <article className="detail-list-item" key={item.id}><b>{item.maintenance_type}</b><span>{formatPortalDate(item.maintenance_date)} · {item.result_status}{item.performed_by ? ` · ${item.performed_by}` : ""}</span>{item.note && <p>{item.note}</p>}</article>) : <p className="empty-records">ยังไม่มีประวัติการดูแล</p>}</div><div className="dashboard-panel"><header><h2>Private work images</h2></header>{media.length ? media.map((item) => <article className="detail-list-item" key={item.id}><Link href={`/api/partner/media/${item.id}`} target="_blank">{item.original_name}</Link><span>{item.content_type} · {Math.ceil(item.size_bytes / 1024)} KB</span></article>) : <p className="empty-records">ไม่มีภาพงานติดตั้ง</p>}</div></section></div>;
}
