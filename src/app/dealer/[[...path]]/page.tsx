...
import { DealerServicePlanEditor, DemoForm, WarrantyQr } from "../../client-ui";
...
import { getDealerProfile, getDealerStats, getDealerWarranties, getDealerWarrantyDetail, type DealerWarrantyDetail as DealerWarrantyRecord, type DealerMaintenanceItem, type DealerMediaItem, type DealerServicePlan, type PortalRecord } from "../../../db/portal-data";
...
export default async function DealerPage({ params, searchParams }: { params: Promise<{ path?: string[] }>; searchParams: Promise<DealerSearchParams> }) {
  ...
  if (section === "profile") {
    ...
    return <DashboardShell role="Dealer" title="ข้อมูลร้านและผู้ติดต่อ" active="profile"><Panel title="Dealer Profile" copy="ข้อมูลร้านจริงจากระบบ โดย Dealer แก้ไขผู้ติดต่อ เบอร์โทร LINE ID และอีเมลได้"><DemoForm kind="profile" profile={profile ? { dealerCode: profile.dealer_code, name: profile.name, province: profile.province, contactName: profile.contact_name, phone: profile.phone, lineId: profile.line_id ?? "", email: profile.email ?? "", certificationTier: profile.certification_tier ?? "", status: profile.status } : undefined} /></Panel></DashboardShell>;
  }
  const [stats, warranties] = await Promise.all([getDealerStats(access.dealerId!), getDealerWarranties(access.dealerId!, 8)]);
  return <DashboardShell role="Dealer" title="ภาพรวมร้านติดตั้ง"><div className="stat-grid"><StatCard label="Active warranties" value={String(stats.primary)} note="ข้อมูลจริงของร้าน" /><StatCard label="Maintenance due" value={String(stats.secondary)} note="ภายใน 30 วัน" /><StatCard label="Total registrations" value={String(stats.quaternary)} note="รายการทั้งหมดของร้าน" /></div><RecordsTable records={warranties} /></DashboardShell>;
}
