...
export default async function AdminPage({ params }: { params: Promise<{ path?: string[] }> }) {
  ...
  if (section === "dealers") return <DashboardShell role="Admin" title="Dealer Management" active="dealers"><div className="admin-stack"><AdminTable records={await getAdminRecords("dealers")} /><AdminPanel title="Create, approve, suspend, or assign account" copy="การสร้างร้านเริ่มที่ pending และต้องเปิดใช้งานก่อนผูกบัญชี Dealer"><AdminDealerForm /></AdminPanel></div></DashboardShell>;
