...
export function WarrantyScanner({ onDetected, onClose }: { onDetected: (value: string) => boolean; onClose: () => void }) {
...
type CustomerData = { customerName: string; customerPhone: string; customerEmail: string; customerLineId: string; vehicleMake: string; vehicleModel: string; vehicleYear: string; vehicleColor: string; vehiclePlate: string; vehicleVinLast6: string; odometerKm: string };
const emptyCustomer: CustomerData = { customerName: "", customerPhone: "", customerEmail: "", customerLineId: "", vehicleMake: "", vehicleModel: "", vehicleYear: "", vehicleColor: "", vehiclePlate: "", vehicleVinLast6: "", odometerKm: "" };

export function CustomerWarrantyForm({ serial, initialData }: { serial: string; initialData?: CustomerData }) {
  const [checking, setChecking] = useState(true);
  const [eligibility, setEligibility] = useState<"ready" | "active" | "not-ready" | "unavailable">("unavailable");
  const [step, setStep] = useState<1 | 2 | 3>(1);
  const [data, setData] = useState<CustomerData>(() => ({ ...emptyCustomer, ...initialData }));
  const [consent, setConsent] = useState(false);
  const [error, setError] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [done, setDone] = useState(false);
  const hasDealerPrefill = Boolean(initialData && Object.values(initialData).some(Boolean));
  ...
  function next(event: FormEvent<HTMLFormElement>) {
    ...
    if (step === 1) {
      ...
      if (data.customerLineId && (data.customerLineId.length > 80 || /\s/.test(data.customerLineId))) return setError("LINE ID ต้องไม่มีช่องว่างและไม่เกิน 80 ตัวอักษร");
    ...
    if (step === 2) {
      ...
      if (!Number.isInteger(year) || year < 1950 || year > new Date().getFullYear() + 1) return setError("กรุณากรอกปีรถเป็น ค.ศ. ให้ถูกต้อง");
      if (data.vehicleColor.trim().length < 2) return setError("กรุณากรอกสีรถก่อน Wrap");
      if (data.vehicleVinLast6 && !/^[A-Za-z0-9]{4,6}$/.test(data.vehicleVinLast6.trim())) return setError("เลขตัวถัง 6 ตัวท้ายต้องเป็นตัวอักษรหรือตัวเลข 4–6 ตัว");
      if (data.odometerKm && (!/^\d+$/.test(data.odometerKm) || Number(data.odometerKm) > 5000000)) return setError("กรุณากรอกเลขไมล์เป็นตัวเลขไม่เกิน 5,000,000 กม.");
  ...
  return <form className="customer-step-form" onSubmit={next} noValidate>
    ...
    {hasDealerPrefill && <p className="customer-prefill-note"><b>ร้านช่วยกรอกข้อมูลบางส่วนไว้แล้ว</b><span>กรุณาตรวจสอบทุกช่อง แก้ไขข้อมูลที่ไม่ถูกต้อง และกรอกส่วนที่ยังว่างก่อนยืนยัน</span></p>}
    {step === 1 && <fieldset><legend>ข้อมูลเจ้าของรถ</legend><p>ใช้สำหรับติดต่อเรื่องสิทธิ์รับประกันเท่านั้น ข้อมูลจะไม่แสดงต่อสาธารณะ</p><label>ชื่อ-นามสกุล<input autoComplete="name" maxLength={120} value={data.customerName} onChange={(e) => update("customerName", e.target.value)} placeholder="ชื่อเจ้าของรถ" required /></label><label>เบอร์โทรศัพท์<input autoComplete="tel" inputMode="tel" maxLength={14} value={data.customerPhone} onChange={(e) => update("customerPhone", e.target.value)} placeholder="08x xxx xxxx" required /></label><label>อีเมล <small>(ไม่บังคับ)</small><input autoComplete="email" inputMode="email" type="email" maxLength={160} value={data.customerEmail} onChange={(e) => update("customerEmail", e.target.value)} placeholder="customer@example.com" /></label><label>LINE ID สำหรับติดต่อ <small>(ไม่บังคับ)</small><input autoComplete="off" maxLength={80} value={data.customerLineId} onChange={(e) => update("customerLineId", e.target.value.trim())} placeholder="LINE ID ไม่ใช่ชื่อแสดงผล" /><small>ใช้เป็นช่องทางสำรองให้ Dealer ติดต่อ ไม่แสดงบนหน้าบัตรสาธารณะ</small></label></fieldset>}
    {step === 2 && <fieldset><legend>ข้อมูลรถที่นำมา Wrap</legend><p>ใช้ยืนยันว่าบัตรนี้เป็นของรถคันใด หน้าสาธารณะจะปกปิดทะเบียนและไม่แสดงเลขตัวถัง</p><label>ยี่ห้อรถ<input autoComplete="off" maxLength={80} value={data.vehicleMake} onChange={(e) => update("vehicleMake", e.target.value)} placeholder="เช่น Porsche" required /></label><label>รุ่นรถ<input autoComplete="off" maxLength={120} value={data.vehicleModel} onChange={(e) => update("vehicleModel", e.target.value)} placeholder="เช่น 911 Carrera" required /></label><label>ปีรถ (ค.ศ.)<input inputMode="numeric" maxLength={4} value={data.vehicleYear} onChange={(e) => update("vehicleYear", e.target.value.replace(/\D/g, ""))} placeholder="เช่น 2025" required /></label><label>สีรถก่อน Wrap<input autoComplete="off" maxLength={60} value={data.vehicleColor} onChange={(e) => update("vehicleColor", e.target.value)} placeholder="เช่น ดำ / ขาว / เทา" required /></label><label>ทะเบียนรถ<input autoComplete="off" maxLength={40} value={data.vehiclePlate} onChange={(e) => update("vehiclePlate", e.target.value)} placeholder="เช่น 1กก 1234 กรุงเทพฯ" required /></label><label>เลขตัวถัง 6 ตัวท้าย <small>(แนะนำ)</small><input autoComplete="off" maxLength={6} value={data.vehicleVinLast6} onChange={(e) => update("vehicleVinLast6", e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, ""))} placeholder="เช่น AB1234" /></label><label>เลขไมล์วันที่รับรถ <small>(ไม่บังคับ)</small><input inputMode="numeric" maxLength={7} value={data.odometerKm} onChange={(e) => update("odometerKm", e.target.value.replace(/\D/g, ""))} placeholder="เช่น 24500 กม." /></label></fieldset>}
    {step === 3 && <fieldset><legend>ตรวจสอบก่อนยืนยัน</legend><dl className="customer-review"><div><dt>เจ้าของรถ</dt><dd>{data.customerName}</dd></div><div><dt>เบอร์โทร</dt><dd>{data.customerPhone}</dd></div><div><dt>LINE ID</dt><dd>{data.customerLineId || "ไม่ได้ระบุ"}</dd></div><div><dt>อีเมล</dt><dd>{data.customerEmail || "ไม่ได้ระบุ"}</dd></div><div><dt>รถ</dt><dd>{data.vehicleMake} {data.vehicleModel} · {data.vehicleYear}</dd></div><div><dt>สีเดิม / ทะเบียน</dt><dd>{data.vehicleColor} · {data.vehiclePlate}</dd></div><div><dt>ยืนยันรถ</dt><dd>{data.vehicleVinLast6 ? `เลขตัวถังท้าย ${data.vehicleVinLast6}` : "ไม่ได้ระบุเลขตัวถัง"}{data.odometerKm ? ` · ${Number(data.odometerKm).toLocaleString("th-TH")} กม.` : ""}</dd></div></dl><label className="customer-consent"><input type="checkbox" checked={consent} onChange={(e) => { setConsent(e.target.checked); setError(""); }} required /><span>ยืนยันว่าข้อมูลรถและงาน Wrap ถูกต้อง และยินยอมให้ NEXS กับศูนย์ติดตั้งใช้ข้อมูลนี้เพื่อออกบัตรและให้บริการรับประกัน ตามนโยบายความเป็นส่วนตัว</span></label></fieldset>}
