...
function WarrantyState({ status, serial, record }: { status: string; serial: string; record?: PublicWarrantyRecord }) {
  ...
  return <div className="card-public-page"><header><Link href="/"><Logo /></Link><Link href="/warranty">ตรวจสอบ Serial อื่น</Link></header><main className="state-card-wrap"><section className={`state-card state-${status}`}><StatusPill status={status} /><p className="eyebrow">SERIAL</p><strong>{serial}</strong><h1>{title}</h1><p>{copy}</p>{record && <dl><CardRow label="Product" value={record.product} /><CardRow label="Status" value={record.expiry} /></dl>}{isNotRegistered && <div className="registration-audience-note"><b>ลูกค้าไม่ต้องเปิดใช้งานบัตรด้วยตนเอง</b><p>Dealer ที่ NEXS อนุมัติจะบันทึกข้อมูลงานติดตั้งและเปิดบัตรให้ หลังจากนั้น QR เดิมจะแสดงสถานะรับประกันอัตโนมัติ</p></div>}<div>{isNotRegistered ? <><ArrowLink href={`/support/warranty?serial=${encodedSerial}`}>ลูกค้า: ติดต่อ Dealer / NEXS</ArrowLink><ArrowLink secondary href={`/dealer/register-warranty?serial=${encodedSerial}`}>Dealer: เข้าสู่ระบบเพื่อลงทะเบียน</ArrowLink></> : <><ArrowLink href="/warranty">ค้นหาอีกครั้ง</ArrowLink><ArrowLink secondary href={`/support/warranty?serial=${encodedSerial}`}>ติดต่อ Support</ArrowLink></>}</div></section></main></div>;
}
